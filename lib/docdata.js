...
export class StyleData {
    ...
    for (const rule of this.#sortedRules) {
      if (matches(node, rule.getSelectorString())) {
        ...
        rule.getDeclarations().forEach((value, name) => {
          if (isDynamic) {
            computedStyles.set(name, unknown);
          } else {
  ...
  /**
   * @param {XastElement} node
   * @param {{element:XastElement,styles?:Map<string,string>}[]} parents
   */
  computeStyle(node, parents) {
    ...
    /**
     * @param {Map<string,string>} currentStyles
     * @param {Map<string,string>} parentStyles
     */
    function mergeMissingProperties(currentStyles, parentStyles) {
      parentStyles.forEach((value, name) => {
        if (!currentStyles.has(name)) {
...
export const getDocData = (root) => {
  ...
  return {
    styles: styleError ? undefined : new StyleData(ruleSets),
  };
};
