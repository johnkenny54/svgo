...
/**
 * @type {(ruleNode: CsstreeRule, dynamic: boolean) => CSSRule[]}
 */
...
/**
 * @type {(css: string, dynamic: boolean) => CSSRuleSet[]}
 */
export const parseStylesheet = (css, dynamic) => {
   ...
   * @returns
   */
  function addRuleSet(atRule, rules) {
    if (rules.length === 0) {
      return rules;
    }
  ...
  csstree.walk(ast, (cssNode) => {
    ...
    if (cssNode.type === 'Atrule') {
      rules = addRuleSet(atRule, rules);
      atRule = `@${cssNode.name} ${cssNode.prelude ? csstree.generate(cssNode.prelude) : ''}`;
